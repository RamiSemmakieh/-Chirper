            ...
            @foreach ($chirps as $chirp)
                <div class="p-6 flex flex-col space-y-2">
                    ...
                    <div class="mt-4 space-y-2">
                        @foreach ($chirp->replies as $reply)
                            <div class="flex items-center space-x-2">
                                ...
                                <small class="text-sm text-gray-600">{{ $reply->created_at->format('j M Y, g:i a') }}</small>
                                <p class="text-lg text-gray-900">{{ $reply->body }}</p>
                            </div>
                        @endforeach
                    </div>
                    <!-- End Display Replies -->
                    ...
                    <div class="flex items-center mt-2">
                        <form method="POST" action="{{ route('replies.store') }}" class="mr-2">
                            @csrf
                            <input type="hidden" name="chirp_id" value="{{ $chirp->id }}">
                            <textarea name="body" placeholder="{{ __('Reply to this Chirp...') }}" class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"></textarea>
                            <x-input-error :messages="$errors->get('body')" class="mt-2" />
                            <x-primary-button class="mt-2">{{ __('Reply') }}</x-primary-button>
                        </form>
